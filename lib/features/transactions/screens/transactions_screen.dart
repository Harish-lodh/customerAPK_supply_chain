...
class _TransactionsScreenState extends State<TransactionsScreen> {
  ...
  Future<void> _initializeData() async {
    ...
    // Only load lender types if not already loaded
    if (transactionsProvider.lenderTypes.isEmpty) {
    ...
    // Only initialize LANs and load transactions if not already done
    if (transactionsProvider.availableLoans.isEmpty) {
        ...
        builder: (context, provider, child) {
          return Column(
            children: [
              ...
              if (provider.lenderTypes.isNotEmpty)
                _buildLenderTypeDropdown(provider),
  ...
  Widget _buildLenderTypeDropdown(TransactionsProvider provider) {
    ...
    final safeSelectedLenderType = provider.lenderTypes
            .contains(provider.selectedLenderType)
        ? provider.selectedLenderType
  ...
  Widget _buildTransactionsList(TransactionsProvider provider) {
    if (provider.state == TransactionsState.loading &&
        provider.collectionTransactions.isEmpty) {
      return const Center(child: CircularProgressIndicator());
    }

    if (provider.state == TransactionsState.error &&
        provider.collectionTransactions.isEmpty) {
      return Center(
        child: Column(
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        padding: const EdgeInsets.all(16),
        itemCount:
            provider.collectionTransactions.length + (provider.hasMore ? 1 : 0),
        itemBuilder: (context, index) {
          if (index >= provider.collectionTransactions.length) {
            provider.loadMore();
            return const Center(child: CircularProgressIndicator());
          }
          return _buildCollectionTransactionCard(
              provider.collectionTransactions[index]);
        },
      ),
    );
  }
  ...
  void _showAllocationBottomSheet(TransactionsProvider provider) {
    ...
    showModalBottomSheet(
      context: context,
      useRootNavigator: true, // 🔥 THIS FIXES IT
        ...
        builder: (context, scrollController) => SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // Allocation Breakup
              const Text('Allocation Breakup',
                  style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
              const SizedBox(height: 12),
              _buildAllocationRow('Principal',
                  detail.allocationBreakup.allocatedPrincipal, currencyFormat),
              _buildAllocationRow('Interest',
                  detail.allocationBreakup.allocatedInterest, currencyFormat),
              _buildAllocationRow(
                  'Penal Interest',
                  detail.allocationBreakup.allocatedPenalInterest,
                  currencyFormat),
              _buildAllocationRow('Excess Payment',
                  detail.allocationBreakup.excessPayment, currencyFormat),
              const Divider(height: 32),

              // Invoice-wise Allocation
              const Text('Invoice-wise Allocation',
                  style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
              const SizedBox(height: 12),
              ...detail.invoiceWiseAllocation.map((invoice) => Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 8),
                        _buildAllocationRow('Principal',
                            invoice.allocatedPrincipal, currencyFormat),
                        _buildAllocationRow('Interest',
                            invoice.allocatedInterest, currencyFormat),
                        _buildAllocationRow('Penal Interest',
                            invoice.allocatedPenalInterest, currencyFormat),
                      ],
                    ),
                  )),
              const SizedBox(height: 20),
